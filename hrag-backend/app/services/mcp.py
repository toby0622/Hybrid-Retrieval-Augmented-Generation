...
class MCPTools:
    ...
    @staticmethod
    async def get_service_health(service_name: str) -> tuple[List[Dict[str, Any]], str]:
        query = """
            SELECT 
                service_name,
                health_status,
            ...
            LIMIT 1
        """
        results = await MCPDatabaseClient.execute_query(query, (f"%{service_name}%",))
        return results, query.strip()

    @staticmethod
    async def query_realtime_data(
        table_name: str, filters: Optional[Dict[str, Any]] = None, limit: int = 20
    ) -> tuple[List[Dict[str, Any]], str]:
        allowed_tables = [
            "service_metrics",
            "service_logs",
